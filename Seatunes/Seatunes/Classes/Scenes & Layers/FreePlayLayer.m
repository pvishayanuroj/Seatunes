...
static const CGFloat FPL_KEYBOARD_X_M = 35.0f;
static const CGFloat FPL_KEYBOARD_Y_M = 31.25f;
...
- (id) init
{
    if ((self = [super init])) {
        ...
        bubbles_.position = ADJUST_IPAD_CCP(ccp(FPL_BUBBLE_X, FPL_BUBBLE_Y));
        [self addChild:bubbles_ z:FPL_BUBBLES_Z];            
        
        instructor_ = [[Instructor instructor:kWhaleInstructor] retain];
        instructor_.position = ADJUST_IPAD_CCP(ccp(FPL_INSTRUCTOR_X, FPL_INSTRUCTOR_Y));
        ...
        keyboard_.position = CHOOSE_REL_CCP(ccp(FPL_KEYBOARD_X, FPL_KEYBOARD_Y), ccp(FPL_KEYBOARD_X_M, FPL_KEYBOARD_Y_M));
        ...
        sideMenu_.position = ADJUST_IPAD_CCP(ccp(FPL_SIDEMENU_X, FPL_SIDEMENU_Y));
        
        [sideMenu_ addMenuBackground:@"Side Parchment.png" pos:ADJUST_IPAD_CCP(ccp(0, FPL_SIDEMENU_SPRITE_Y))];
...
{ 
    if (!sideMenuMoving_) {
        ...
        CCActionInterval *move = [CCMoveBy actionWithDuration:FPL_SIDEMENU_MOVE_TIME position:ADJUST_IPAD_CCP(ccp(-FPL_SIDEMENU_MOVE_AMOUNT, 0))];
