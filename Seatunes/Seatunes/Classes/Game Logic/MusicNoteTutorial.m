...
static const CGFloat MNT_KEYBOARD_X_M = 35.0f;
static const CGFloat MNT_KEYBOARD_Y_M = 31.25f;
...
- (id) initMusicNoteTutorial
{
    if ((self = [super initGameLogic:kDifficultyMedium])) {
        ...
        staff_.position = ADJUST_IPAD_CCP(ccp(MNT_STAFF_X, MNT_STAFF_Y));
        [staff_ disableLoop];
        ...
        instructor_.position = ADJUST_IPAD_CCP(ccp(MNT_INSTRUCTOR_X, MNT_INSTRUCTOR_Y));
        [self addChild:instructor_];            
        
        keyboard_ = [[Keyboard keyboard:kEightKey] retain];
        keyboard_.delegate = self;
        keyboard_.isClickable = NO;        
        keyboard_.position = CHOOSE_REL_CCP(ccp(MNT_KEYBOARD_X, MNT_KEYBOARD_Y), ccp(MNT_KEYBOARD_X_M, MNT_KEYBOARD_Y_M));
        ...
        reader_.position = ADJUST_IPAD_CCP(ccp(MNT_INSTRUCTOR_X + MNT_READER_OFFSET_X, MNT_INSTRUCTOR_Y + MNT_READER_OFFSET_Y));
