...
- (id) initGameLogicD:(NSString *)songName
{
    if ((self = [super initGameLogic:kDifficultyEasy])) {
        ...
        instructor_.position = ADJUST_IPAD_CCP(ccp(GLD_INSTRUCTOR_X, GLD_INSTRUCTOR_Y));
        [self addChild:instructor_ z:GLD_INSTRUCTOR_Z];        
        
        bubbles_ = [[BubbleGroup bubbleGroupWithBubbles:0.02f] retain];
        bubbles_.position = ADJUST_IPAD_CCP(ccp(GLD_BUBBLE_X, GLD_BUBBLE_Y));
        ...
        reader_.position = ADJUST_IPAD_CCP(ccp(GLD_INSTRUCTOR_X + GLD_READER_OFFSET_X, GLD_INSTRUCTOR_Y + GLD_READER_OFFSET_Y));
