...
static const CGFloat GLF_KEYBOARD_X_M = 35.0f;
static const CGFloat GLF_KEYBOARD_Y_M = 31.25f;
...
- (id) initGameLogicF:(NSString *)songName
{
    if ((self = [super initGameLogic:kDifficultyMedium])) {
        ...
        instructor_.position = ADJUST_IPAD_CCP(ccp(GLF_INSTRUCTOR_X, GLF_INSTRUCTOR_Y));
        [self addChild:instructor_ z:GLF_INSTRUCTOR_Z];           
        
        bubbles_ = [[BubbleGroup bubbleGroupWithBubbles:0.02f] retain];
        bubbles_.position = ADJUST_IPAD_CCP(ccp(GLF_BUBBLE_X, GLF_BUBBLE_Y));
        ...
        keyboard_.position = CHOOSE_REL_CCP(ccp(GLF_KEYBOARD_X, GLF_KEYBOARD_Y), ccp(GLF_KEYBOARD_X_M, GLF_KEYBOARD_Y_M));
        ...
        reader_.position = ADJUST_IPAD_CCP(ccp(GLF_INSTRUCTOR_X + GLF_READER_OFFSET_X, GLF_INSTRUCTOR_Y + GLF_READER_OFFSET_Y));
