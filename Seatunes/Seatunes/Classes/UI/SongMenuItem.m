...
- (id) initSongMenuItem:(NSString *)songName scores:(NSDictionary *)scores songIndex:(NSUInteger)songIndex hasScore:(BOOL)hasScore locked:(BOOL)locked
{
    if ((self = [super initScrollingMenuItem:songIndex height:CHOOSE_REL_CCP(SMI_CELL_HEIGHT, SMI_CELL_HEIGHT_M)])) {
        ...
        label_.position = ADJUST_IPAD_CCP(ccp(SMI_SONG_NAME_X, 0));
        ...
        if (hasScore) {
            ...
            easySprite.position = ADJUST_IPAD_CCP(ccp(SMI_STAR_X + 0 * SMI_STAR_PADDING, 0));        
            mediumSprite.position = ADJUST_IPAD_CCP(ccp(SMI_STAR_X + 1 * SMI_STAR_PADDING, 3));        
            hardSprite.position = ADJUST_IPAD_CCP(ccp(SMI_STAR_X + 2 * SMI_STAR_PADDING, 0));
            ...
            sprite.position = ADJUST_IPAD_CCP(ccp(SMI_STAR_X + 1 * SMI_STAR_PADDING, 0));
