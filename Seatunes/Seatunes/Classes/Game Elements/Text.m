...
static const CGFloat TXT_DEFAULT_WIDTH_M = 215.0f;
static const CGFloat TXT_DEFAULT_HEIGHT_M = 16.0f;
static const CGFloat TXT_DEFAULT_SPACE_WIDTH_M = 4.68f;
...
+ (id) text:(NSString *)text fntFile:(NSString *)fntFile
{
    if (IS_IPAD()) {
        ...
        return [[[self alloc] initText:text fntFile:fntFile width:TXT_DEFAULT_WIDTH_M height:TXT_DEFAULT_HEIGHT_M spaceWidth:TXT_DEFAULT_SPACE_WIDTH_M] autorelease];        
    }
}
...
+ (id) text:(NSString *)text fntFile:(NSString *)fntFile width:(CGFloat)width
{
    if (IS_IPAD()) {
        ...
        return [[[self alloc] initText:text fntFile:fntFile width:width height:TXT_DEFAULT_HEIGHT_M spaceWidth:TXT_DEFAULT_SPACE_WIDTH_M] autorelease];            
    }
}
...
- (void) updateText
{
    ...
    for (CCLabelBMFont *label in components_) {
        ...
        // Add the label + space
        pos.x += (label.contentSize.width + spaceWidth_);
    }
